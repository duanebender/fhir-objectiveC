...
- (id)init
{
    ...
    if (self) {
        ...
        _when = [[Code alloc] init];
        _duration = [[NSNumber alloc] init];
        _units = [[Code alloc] init];
...
- (NSDictionary *)generateAndReturnDictionary
{
    ...
    _scheduleDictionary.dataForResource = [NSDictionary dictionaryWithObjectsAndKeys:
                                          [ExistanceChecker generateArray:_event], @"event",
...
- (void)scheduleParser:(NSDictionary *)dictionary
{
    ...
    _end = [ExistanceChecker generateDateTimeFromString:[_repeat.dataForResource objectForKey:@"end"]];
}
