...
#define ARRAY_STRINGS [NSSet setWithObjects:@"xs:documentation",nil] //strings that need to have internal checked

NSString *const kXMLReaderTextNodeKey = @"value";

@interface XSDReader (Internal)
...
{
    XSDReader *reader = [[XSDReader alloc] initWithError:error];
...
+ (NSDictionary *)dictionaryForXMLString:(NSString *)string error:(NSError *)error
{
    ...
    return [XSDReader dictionaryForXMLData:data error:error];
}
...
- (void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if ([textInProgress length] > 0 && ([dictInProgress objectForKey:@"xs:documentation"]))
    {
        NSString *finalString = [textInProgress stringByTrimmingCharactersInSet:
                                   [NSCharacterSet whitespaceAndNewlineCharacterSet]];
        //NSLog(@"%@",finalString);
        
        //add to dictionary final edited string
        [dictInProgress setObject:finalString forKey:@"xs:documentation"];//kXMLReaderTextNodeKey];
