...
@property (nonatomic, retain) FHIRCoding *gender; //Administrative Gender - the gender that the patient is considered to have for administration / record keeping purposes
@property (nonatomic, retain) FHIRString *birthDate; //The birth date for the individual, to the degre of precision now
@property (nonatomic) FHIRBool *deceased; //Indicates if the individual is deceased or not
@property (nonatomic, retain) NSMutableArray *address; //One or more addresses for the individual. An array of Addresses.
@property (nonatomic, retain) FHIRCodeableConcept *maritalStatus; //This field contains a patient's marital (civil) status.
